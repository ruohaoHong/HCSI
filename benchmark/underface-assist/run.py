    ...
    for case in cases:
        ...
        if scale.px_per_cm is not None:
            ...
            if profile is not None:
                original_landmark = estimate_head_underface(profile)
                independent = inspect_bearing_face(profile)
                structure = {
                    "baseline_first_expansion_px": None if original_landmark is None else abs(profile.tip_s - original_landmark.s),
                    "bearing_face_evidence": dataclasses.asdict(independent),
                    "assistive_L_mm": (
                        None if independent.bearing_plane_s is None
                        else round(abs(profile.tip_s - independent.bearing_plane_s)/effective * 10, 3)
                    ),
                }
        steps = result['geometry_steps']
        values = {s['purpose']: s['value_mm'] for s in steps}
        # The auxiliary detector MUST NOT change ANY existing D/P/L result.
        # This is an ablation against the exact accepted 124da baseline,
        # not a test of the later experimental bearing-plane branch.
        for dimension in ('D', 'P', 'L'):
            if values[dimension] != case['baseline'][dimension]:
                raise AssertionError(f"{case['case']}: baseline {dimension} changed: {values}")
        ...
        record = dict(case=case['case'], source=case['source'], image_url=case['image_url'],
                      ...
                      controlled_semantics=True, api_calls=0, result=result)
        records.append(record)
        print(json.dumps({k: record[k] for k in ('case', 'values_mm', 'errors_pct', 'structure')}, ensure_ascii=False))
