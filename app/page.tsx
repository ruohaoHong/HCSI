      ...
      <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
        ...
        <div aria-live="polite">
          ...
          {state&&<div className="space-y-4">
            ...
            {state.diagnostic&&<div className="rounded-xl border border-dashed border-border bg-muted/20 p-5"><h3 className="text-sm font-semibold">TEST DIAGNOSTIC · 尺度／幾何</h3><div className="mt-3 grid gap-2 text-xs leading-5 sm:grid-cols-2"><p><span className="text-muted-foreground">尺度參照：</span>{state.diagnostic.scale_reference_present?'有':'無'}</p><p><span className="text-muted-foreground">使用相對幾何：</span>{state.diagnostic.relative_geometry_used?'有':'無'}</p><p><span className="text-muted-foreground">可見螺紋跨度：</span>{state.diagnostic.visible_thread_span||'—'}</p><p><span className="text-muted-foreground">估計牙數：</span>{state.diagnostic.estimated_thread_count??'—'}</p><p><span className="text-muted-foreground">直徑 / 牙距：</span>{state.diagnostic.diameter_to_pitch_ratio||'—'}</p><p><span className="text-muted-foreground">長度 / 直徑：</span>{state.diagnostic.length_to_diameter_ratio||'—'}</p></div>{state.diagnostic.strongest_system_evidence&&<p className="mt-3 text-xs leading-5"><span className="text-muted-foreground">最強制式證據：</span>{state.diagnostic.strongest_system_evidence}</p>}{state.diagnostic.scale_observations?.length>0&&<div className="mt-3 border-t border-border/60 pt-3"><p className="text-xs font-semibold text-muted-foreground">尺度觀察</p><ul className="mt-2 space-y-1 text-xs leading-5">{state.diagnostic.scale_observations.map((item,index)=><li key={index}>• {item}</li>)}</ul></div>}</div>}
