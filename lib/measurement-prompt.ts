...
規則：
1. 可以把 length_mm / width_mm 與 status=measured 的 geometry_steps 當作本次照片的外部實測證據，用來排除尺寸不相容的候選。
2. geometry_steps 中 status=not_measured 的項目不是證據，不得補猜其數值。
3. 每個 geometry step 的 value_mm 只代表該 operation + inputs 所定義的物理距離；請依 purpose 解讀，不要把 axial_distance 自動改名成其他規格。
4. 不可自行修改實測值、四捨五入成另一個標準規格後宣稱照片已證明該標準規格；標準規格仍需結合外觀與 reference 判斷。
5. 不可由這些量測值推導照片沒有實際量到的牙距、孔徑、螺紋規格或其他尺寸。
6. 若你在 specifications 引用量測值，label 請明確寫「系統實測…」，不要把它描述成 AI 目測。
`
  }
