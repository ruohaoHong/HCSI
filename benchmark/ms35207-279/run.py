...
if contour is not None:
    ...
    record["frame_comparison"]={
        "legacy":None if legacy is None else {
            "axis":legacy.axis,"transition_s":legacy.transition_s,
            "tip_s":legacy.tip_s,
        },
        "local":None if local is None else {
            "axis":local.axis,"transition_s":local.transition_s,
            "tip_s":local.tip_s,
        },
        "selected":None if profile is None else {
            "axis":profile.axis,"transition_s":profile.transition_s,
            "tip_s":profile.tip_s,
        },
    }
    if local is None:
        raise AssertionError("local shaft regime not found in MS35207-279 real photo")
    if abs(float(local.axis[0])) < 0.94:
        raise AssertionError(f"local shaft axis is not aligned with the visible horizontal screw: {local.axis}")
