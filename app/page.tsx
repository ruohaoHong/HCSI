...
export default function Page() {
  ...
  async function analyze() {
    ...
    setError('')
    try {
      const response = await fetch('/api/analyze', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ image: imageData }) })
        ...
        <section className="grid flex-1 gap-10 py-10 lg:grid-cols-[0.82fr_1.18fr] lg:items-center lg:gap-20 lg:py-16">
          ...
          <div className="relative">
            ...
            <div className="relative overflow-hidden rounded-xl border border-border bg-card shadow-xl shadow-primary/5">
              ...
              <input ref={inputRef} className="sr-only" type="file" accept="image/*" capture="environment" onChange={handleFile} />
              {imageUrl && <div className="flex flex-col gap-3 border-t border-border p-4 sm:flex-row"><button type="button" onClick={() => inputRef.current?.click()} className="flex flex-1 items-center justify-center gap-2 rounded-md border border-border px-4 py-3 text-sm font-medium transition-colors hover:bg-muted"><RotateCcw size={15} /> 重新拍攝</button><button type="button" onClick={analyze} disabled={phase === 'processing'} className="flex flex-[1.5] items-center justify-center gap-2 rounded-md bg-accent px-4 py-3 text-sm font-semibold text-accent-foreground transition-all hover:brightness-95 disabled:cursor-wait disabled:opacity-70">{phase === 'processing' ? <><Loader2 size={16} className="animate-spin" /> 正在辨識...</> : <><Sparkles size={16} /> 開始辨識 <ArrowUpRight size={15} /></>}</button></div>}
            </div>
          </div>
        </section>

        <section aria-live="polite" className="pb-10">{phase === 'processing' && <div className="rounded-xl border border-border bg-card p-5"><div className="mb-4 flex items-center gap-3"><div className="grid size-8 place-items-center rounded-full bg-accent/15 text-accent"><Loader2 size={16} className="animate-spin" /></div><div><p className="text-sm font-semibold">AI 正在檢視元件特徵</p><p className="text-xs text-muted-foreground">比對形狀、比例與表面特徵...</p></div></div><div className="space-y-2"><div className="h-2 animate-pulse rounded bg-muted" /><div className="h-2 w-4/5 animate-pulse rounded bg-muted" /></div></div>}{phase === 'success' && <div className="result-card rounded-xl border border-accent/30 bg-card p-5 sm:p-7"><div className="mb-5 flex items-start justify-between gap-4"><div className="flex items-center gap-3"><div className="grid size-9 place-items-center rounded-full bg-accent/15 text-accent"><Check size={18} /></div><div><p className="font-mono text-[10px] uppercase tracking-widest text-accent">Analysis complete</p><h2 className="mt-1 text-lg font-semibold">辨識結果</h2></div></div><ShieldCheck size={19} className="text-muted-foreground" aria-label="安全輸出" /></div><pre className="whitespace-pre-wrap font-sans text-sm leading-7 text-card-foreground">{result || sampleResult}</pre><p className="mt-5 border-t border-border pt-4 text-xs leading-5 text-muted-foreground">AI 辨識結果僅供參考，實際尺寸請使用量具確認。</p></div>}{phase === 'error' && <div className="flex items-start gap-3 rounded-xl border border-destructive/30 bg-card p-5 text-sm"><AlertCircle size={18} className="mt-0.5 shrink-0 text-destructive" /><div><p className="font-semibold">辨識未完成</p><p className="mt-1 text-muted-foreground">{error}</p></div></div>}</section>
